...
<body>
  ...
  <div class="collapse navbar-collapse" id="conteudoNavbarSuportado">
    ...
    <form class="form-inline my-2 my-lg-0 mr-5">
      <input name="txtpesquisaridade" id="txtidade" class="form-control mr-sm-2" type="search" placeholder="Buscar pela idade" aria-label="Pesquisar">
      <button name="buttonPesquisaridade" class="btn btn-outline-success my-2 my-sm-0" type="submit"><i class="fa fa-search"></i></button>
    </form>
                      ...
                      <?php
                        ...
                        if(isset($_GET['buttonPesquisar']) and $_GET['txtpesquisar'] != ''){
                          $nome = $_GET['txtpesquisar'] . '%';
                           $query = "select * from animais where nome LIKE '$nome'  order by nome asc"; 
                           // novo codigo ( procurar animal pela idade)
                        }else if(isset($_GET['buttonPesquisaridade']) and $_GET['txtpesquisaridade'] != ''){
                          $nome = $_GET['txtpesquisaridade'];
                           $query = "select * from animais where idade = '$nome'  order by idade asc"; 
                        }
                        // novo codigo ( procurar ocorrência pela data)
                        else if(isset($_GET['buttonPesquisardata']) and $_GET['txtpesquisardata'] != ''){
                           $nome = $_GET['txtpesquisardata'];
                           $query = "select * from animais where data = '$nome'  order by data asc"; 
                        }
                        // novo codigo ( procurar animais adotados)
                     ...
                     // novo codigo ( procurar por animais não adotados)
                      ...
                      $query = "select * from animais where vacinas = '$nome'   order by data asc"; 
                        ...
                        //final do código
                         ...
                         $query = "select * from animais order by id desc"; 
                        }
                        ...
                        <tbody>
                         ...
                         <?php 
                          ...
                          while($res_1 = mysqli_fetch_array($result)){
                            ...
                            $id = $res_1["id"];
                            $nomeexcluido = $nome; // Variavel criada somente para enviar LOG do nome que foi excluído
                            ...
                            <?php 
                              ...
                              ?>
                              <?php 
                              if($situacao == "Naodisponivel"): ?>
                                  <td class="badge badge-pill badge-danger" ><?php echo "Não disponível"; ?></td>
                            <?php 
                              endif;
...
?>
 ...
 <script> $("#modaladotar").modal("show"); </script> 
