...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "SELECT p.nome AS nome_usuario, d.idusuario, d.caminho 
              FROM propostas p
              JOIN documentos d ON p.idusuario = d.idusuario
              WHERE p.nome LIKE '%$nomePesquisado%'";
                ...
                <tbody>
                    ...
                    if (isset($result)) {
                        while ($row = mysqli_fetch_assoc($result)) {
                            $nomeUsuario = $row['nome_usuario']; // Correto: $nomeUsuario
                            $documentoAnexado = $row['caminho'];

                            echo "<tr>";
                            echo "<td>$nomeUsuario</td>"; // Correto: $nomeUsuario
